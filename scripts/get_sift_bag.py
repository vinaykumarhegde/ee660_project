#! /usr/bin/env python
# Python script to generate SIFT features and
# extract bag of features out of them.
# Stores the dictionary of SIFT features in hdf5 file
...
sift=cv2.SIFT(nOctaveLayers=4,contrastThreshold=0.01, edgeThreshold=20, sigma=1.2)
...
siftfile='../dataset/'+cfg['defaults']['dataset']+'/dataset_sift2.hdf5'
datfile=h5py.File(siftfile,'w')

# Store Train Data:
count_train=0
train_data=datfile.create_dataset('train_data',(0,128),maxshape=(None,128)) #SIFT features.
...
for i,img in enumerate(_X_train_c):#Process every row in X_train
    ...
    _, des = sift.detectAndCompute(img_gray,None) #get the SIFT descriptors
    try:
...
# Store test data:
count_test=0
test_data=datfile.create_dataset('test_data',(0,128),maxshape=(None,128)) #SIFT features.
...
print "------- Statistics ---------"
print count_train, "train images don't have sift features"
print count_test, "test images don't have sift features"
